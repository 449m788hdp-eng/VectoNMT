...
type ActiveTest = { attemptId: string; subject: string; subjectName: string; mode: "quick" | "full"; durationMinutes: number | null; questions: Question[] };
type TestResult = { score: number; correct: number; total: number; rawScore: number; rawMax: number };
...
function TestsView({ stats, onStart }: { stats: SubjectStat[]; onStart: (slug: string, mode?: "quick" | "full") => void }) {
  return <section><p className="text-sm font-medium text-[#c8ff38]">Банк · 2 512 завдань</p><h1 className="mt-2 text-4xl font-semibold tracking-[-.045em] sm:text-5xl">Обери предмет</h1><p className="mt-3 max-w-3xl leading-7 text-white/40">412 офіційних завдань НМТ-2025 і 2 100 оригінальних тренувальних вправ за темами програми. Повна симуляція відтворює кількість і типи завдань НМТ-2026.</p><div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{subjects.map(({ slug, name, icon: Icon, count, tone }) => { const stat = stats.find((item) => item.slug === slug); return <article key={slug} className="rounded-[24px] border border-white/8 bg-white/[.025] p-5 transition hover:-translate-y-1 hover:border-white/20"><div className="flex items-center justify-between"><span className={`grid size-11 place-items-center rounded-full subject-bg-${tone}`}><Icon className="size-5" /></span><span className="rounded-full border border-white/8 px-3 py-1 text-xs text-white/35">60 хв</span></div><h2 className="mt-7 text-xl font-medium">{name}</h2><p className="mt-1 text-sm text-white/32">{count} завдань у повному тесті</p><div className="mt-5 grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => onStart(slug, "quick")} className="rounded-full border-white/10 bg-transparent text-white hover:bg-white/10 hover:text-white">Швидкий · 10</Button><Button onClick={() => onStart(slug, "full")} className="rounded-full bg-[#c8ff38] text-black hover:bg-[#d5ff65]">Повний · {count}</Button></div><div className="mt-5 flex items-center justify-between border-t border-white/8 pt-4 text-sm"><span className="text-white/35">Пройдено: {stat?.tests ?? 0}</span><span className="text-[#c8ff38]">{stat?.accuracy == null ? "Новий" : `${stat.accuracy}%`}</span></div></article>; })}</div></section>;
}
...
function TestPlayer({ test, onClose, onCompleted }: { test: ActiveTest; onClose: () => void; onCompleted: (result: TestResult) => void }) {
  ...
  const answerControl = question.question_type === "numeric" ? <Input inputMode="decimal" value={selected} onChange={(event) => setAnswers((current) => ({ ...current, [question.id]: event.target.value }))} placeholder="Уведи числову відповідь" className="mt-8 h-14 rounded-2xl border-white/10 bg-white/[.035] px-4 text-lg text-white" /> : question.question_type === "matching" ? <div className="mt-8 space-y-3">{groups[0]?.options.map((left) => <div key={left.marker} className="grid gap-3 rounded-2xl border border-white/9 bg-white/[.025] p-4 sm:grid-cols-[1fr_240px] sm:items-center"><p className="text-white/72"><strong className="mr-2 text-[#c8ff38]">{left.marker}</strong>{left.text}</p><select aria-label={`Відповідь для ${left.marker}`} value={pairs[left.marker] ?? ""} onChange={(event) => setPair(left.marker, event.target.value)} className="h-11 rounded-xl border border-white/10 bg-[#171a18] px-3 text-white"><option value="">Обрати пару</option>{groups[1]?.options.map((right) => <option key={right.marker} value={right.marker}>{right.marker} — {right.text}</option>)}</select></div>)}</div> : question.question_type === "ordering" ? <div className="mt-8 space-y-3">{options.map((_, order) => <div key={order} className="grid grid-cols-[88px_1fr] items-center gap-3"><span className="text-sm text-white/38">{order + 1} місце</span><select value={pairs[String(order + 1)] ?? ""} onChange={(event) => setPair(String(order + 1), event.target.value)} className="h-12 rounded-xl border border-white/10 bg-[#171a18] px-3 text-white"><option value="">Обрати подію</option>{options.map((option) => <option key={option.marker} value={option.marker}>{option.marker} — {option.text}</option>)}</select></div>)}</div> : question.question_type === "type_6" ? <div className="mt-8 space-y-5">{groups.map((group, groupIndex) => <div key={group.title} className="rounded-2xl border border-white/9 bg-white/[.025] p-4"><p className="mb-3 font-medium text-white/70">{group.title}</p><div className="grid gap-2">{group.options.map((option) => <button key={option.marker} onClick={() => setGrouped(groupIndex, option.marker)} className={`rounded-xl border px-4 py-3 text-left ${selected.split(";")[groupIndex] === markerKey(option.marker) ? "border-[#c8ff38]/60 bg-[#c8ff38]/10" : "border-white/8"}`}>{option.marker} — {option.text}</button>)}</div></div>)}</div> : <div className="mt-8 grid gap-3">{options.map((option) => { const chosen = question.question_type === "multiple_choice" ? selected.split(";").includes(markerKey(option.marker)) : selected === option.marker; return <button key={option.marker} onClick={() => question.question_type === "multiple_choice" ? toggleMultiple(option.marker) : setAnswers((current) => ({ ...current, [question.id]: option.marker }))} className={`flex min-h-14 items-center gap-4 rounded-2xl border px-4 py-3 text-left transition ${chosen ? "border-[#c8ff38]/60 bg-[#c8ff38]/10" : "border-white/9 bg-white/[.025] hover:border-white/20"}`}><span className={`grid size-8 shrink-0 place-items-center rounded-full text-sm font-semibold ${chosen ? "bg-[#c8ff38] text-black" : "bg-white/8 text-white/55"}`}>{chosen && question.question_type === "multiple_choice" ? <Check className="size-4" /> : option.marker}</span><span className="leading-6 text-white/75">{option.text || "Варіант із зображенням"}</span>{option.images?.map((src) => <img key={src} src={src} alt="Варіант відповіді" className="ml-auto max-h-20 max-w-32 rounded bg-white object-contain p-1" />)}</button>; })}</div>;
  return <main className="min-h-screen bg-[#080a09] px-4 py-5 text-white sm:px-8"><div className="mx-auto max-w-4xl"><header className="flex items-center justify-between"><button onClick={onClose} className="flex items-center gap-2 text-sm text-white/45 hover:text-white"><ArrowLeft className="size-4" /> Вийти</button><div className="flex items-center gap-3 text-sm"><span className="text-white/38">{index + 1} / {test.questions.length}</span>{test.mode === "full" && <span className="rounded-full border border-[#c8ff38]/20 bg-[#c8ff38]/8 px-3 py-1 font-mono text-[#c8ff38]">{String(Math.floor(secondsLeft / 60)).padStart(2, "0")}:{String(secondsLeft % 60).padStart(2, "0")}</span>}</div></header><Progress value={((index + 1) / test.questions.length) * 100} className="mt-5 h-1.5 bg-white/8 [&_[data-slot=progress-indicator]]:bg-[#c8ff38]" /><article className="mt-8 rounded-[30px] border border-white/10 bg-[#111412] p-5 sm:p-9"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-[#c8ff38]">{test.subjectName} · {question.topic}</p><span className="text-xs uppercase tracking-[.12em] text-white/25">{question.question_type.replaceAll("_", " ")}</span></div><h1 className="mt-5 whitespace-pre-line text-xl font-medium leading-8 sm:text-2xl">{question.prompt.replaceAll("\\\\(", "").replaceAll("\\\\)", "")}</h1>{images.length > 0 && <div className="mt-6 grid gap-3 sm:grid-cols-2">{images.map((src) => <img key={src} src={src} alt="Ілюстрація до завдання" className="max-h-72 rounded-2xl bg-white object-contain p-2" />)}</div>}{answerControl}<div className="mt-8 flex items-center justify-between"><Button variant="ghost" disabled={index === 0} onClick={() => setIndex((value) => value - 1)} className="rounded-full text-white/50 hover:bg-white/10 hover:text-white">Назад</Button>{index < test.questions.length - 1 ? <Button disabled={!ready} onClick={() => setIndex((value) => value + 1)} className="h-12 rounded-full bg-white px-6 text-black hover:bg-white/85">Далі <ArrowRight className="size-4" /></Button> : <Button disabled={!ready || sending} onClick={finish} className="h-12 rounded-full bg-[#c8ff38] px-6 text-black hover:bg-[#d5ff65]">{sending ? "Перевіряю…" : "Завершити тест"}</Button>}</div></article><p className="mt-5 text-center text-xs text-white/22">{question.attribution}</p></div></main>;
}

function ResultScreen({ result, onContinue }: { result: TestResult; onContinue: () => void }) {
  return <main className="grid min-h-screen place-items-center bg-[#080a09] p-5 text-white"><div className="w-full max-w-lg rounded-[32px] border border-white/10 bg-[#111412] p-8 text-center"><span className="mx-auto grid size-16 place-items-center rounded-full bg-[#c8ff38]/10 text-[#c8ff38]"><Trophy className="size-7" /></span><p className="mt-7 text-sm text-white/38">Тренувальний бал</p><h1 className="mt-2 text-7xl font-semibold tracking-[-.07em] text-[#c8ff38]">{result.score}</h1><p className="mt-4 text-lg text-white/55">{result.rawScore} тестових балів із {result.rawMax}</p><p className="mt-1 text-sm text-white/35">повністю виконано {result.correct} із {result.total} завдань</p><p className="mt-6 text-sm leading-6 text-white/32">Vekto враховує часткові бали у відповідностях і множинному виборі. Оцінка 100–200 є тренувальною; офіційний результат визначає УЦОЯО.</p><Button onClick={onContinue} className="mt-8 h-13 w-full rounded-full bg-white text-black hover:bg-white/85">До результатів <ArrowRight className="size-4" /></Button></div></main>;
}

function Dashboard({ data, view, setView, onStart, onRefresh, signOutPath, signedIn }: { data: DashboardData; view: View; setView: (view: View) => void; onStart: (slug: string, mode?: "quick" | "full") => void; onRefresh: () => void; signOutPath: string; signedIn: boolean }) {
...
export default function VektoApp({ initialIdentity, signOutPath }: { initialIdentity: { userId: string; email: string } | null; signInPath: string; signOutPath: string }) {
  ...
  const startTest = async (slug: string, mode: "quick" | "full" = "quick") => {
    setLoadingTest(true);
    const response = await fetch("/api/tests/start", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ subject: slug, mode, limit: 10 }) });
