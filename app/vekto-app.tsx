...
function Brand() {
  return <div className="flex items-center gap-3"><span className="grid size-10 place-items-center border border-white/15 bg-white/[.04]"><span className="size-2.5 bg-[#c8ff38] shadow-[0_0_18px_#c8ff38]" /></span><span className="text-[15px] font-semibold tracking-[.16em]">VEKTO</span></div>;
}
...
function Landing({ onStart }: { onStart: () => void }) {
  return <main className="relative min-h-screen overflow-hidden bg-[#080a09] text-white">
    ...
    <header className="relative z-10 mx-auto flex max-w-[1440px] items-center justify-between px-5 py-5 sm:px-8 lg:px-12"><Brand /><span className="border border-white/10 bg-black/20 px-3 py-2 text-sm text-white/45">Гостьовий режим</span></header>
...
function TestsView({ stats, onStart }: { stats: SubjectStat[]; onStart: (slug: string, mode?: "quick" | "full" | "topic", topicId?: number) => void }) {
  ...
  if (selectedSubject) {
    ...
    return <section>
      ...
      <div className="mt-6 overflow-hidden border border-white/10 bg-[#101311] shadow-[0_30px_100px_rgba(0,0,0,.28)]">
  ...
  return <section>
    ...
    <div className="mt-8 grid border-l border-t border-white/10 sm:grid-cols-2 xl:grid-cols-3">{subjects.map(({ slug, name, short, icon: Icon, count, tone }, index) => { const stat = stats.find((item) => item.slug === slug); return <article key={slug} className="group relative min-h-[290px] border-b border-r border-white/10 bg-[#0e110f] p-6 transition hover:bg-[#131713] sm:p-7"><div className="flex items-start justify-between"><span className={`grid size-11 place-items-center subject-bg-${tone}`}><Icon className="size-5" /></span><span className="font-mono text-xs text-white/22">{String(index + 1).padStart(2, "0")} / 07</span></div><p className="mt-10 text-xs font-semibold tracking-[.16em] text-white/28">{short} · 60 ХВ</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.03em]">{name}</h2><p className="mt-2 text-sm text-white/35">{count} завдань у повному варіанті</p><div className="mt-7 flex items-center gap-2"><Button variant="outline" onClick={() => openSubject(slug)} className="h-11 flex-1 rounded-none border-white/12 bg-transparent text-white hover:border-white/25 hover:bg-white/[.05] hover:text-white">Програма</Button><Button onClick={() => onStart(slug, "full")} className="h-11 flex-1 rounded-none bg-white text-black hover:bg-[#c8ff38]">Почати</Button></div><div className="absolute inset-x-6 bottom-5 flex justify-between border-t border-white/8 pt-3 text-xs"><span className="text-white/28">Спроб: {stat?.tests ?? 0}</span><span className="text-[#c8ff38]">{stat?.accuracy == null ? "Ще без результату" : `${stat.accuracy}% точності`}</span></div></article>; })}</div>
    <div className="mt-5 grid gap-3 md:grid-cols-3"><div className="flex items-center gap-3 border border-white/8 bg-white/[.02] p-4"><Database className="size-5 text-[#c8ff38]" /><p className="text-sm text-white/48"><strong className="block text-white/80">Файлова база</strong>Усі питання вже збережені</p></div><div className="flex items-center gap-3 border border-white/8 bg-white/[.02] p-4"><LockKeyhole className="size-5 text-[#c8ff38]" /><p className="text-sm text-white/48"><strong className="block text-white/80">Без ШІ під час тесту</strong>Лише миттєва SQL-вибірка</p></div><div className="flex items-center gap-3 border border-white/8 bg-white/[.02] p-4"><CheckCircle2 className="size-5 text-[#c8ff38]" /><p className="text-sm text-white/48"><strong className="block text-white/80">Формат НМТ-2026</strong>Кількість і типи завдань</p></div></div>
  </section>;
}
...
function ResultScreen({ result, onContinue }: { result: TestResult; onContinue: () => void }) {
  ...
  return <main className="grid min-h-screen place-items-center bg-[#080a09] p-5 text-white"><div className="w-full max-w-lg border border-white/10 bg-[#111412] p-8 text-center"><span className="mx-auto grid size-16 place-items-center border border-[#c8ff38]/20 bg-[#c8ff38]/10 text-[#c8ff38]"><Trophy className="size-7" /></span><p className="mt-7 text-sm text-white/38">{official ? "Результат за таблицею НМТ-2026" : "Тренувальний індекс"}</p><h1 className="mt-2 text-7xl font-semibold tracking-[-.07em] text-[#c8ff38]">{result.score ?? "—"}</h1>{official && !result.passed && <p className="mt-3 font-medium text-amber-200">Пороговий бал не подолано</p>}<p className="mt-4 text-lg text-white/55">{result.rawScore} тестових балів із {result.rawMax}</p><p className="mt-1 text-sm text-white/35">повністю виконано {result.correct} із {result.total} завдань</p><p className="mt-6 text-sm leading-6 text-white/32">{official ? "Переведення 100–200 виконано за чинною таблицею МОН для НМТ-2026. Часткові бали нараховано за офіційними схемами УЦОЯО." : "Для тренування окремої теми показано індекс 100–200; офіційна таблиця застосовується лише до повного предметного тесту."}</p><Button onClick={onContinue} className="mt-8 h-13 w-full rounded-none bg-white text-black hover:bg-[#c8ff38]">До результатів <ArrowRight className="size-4" /></Button></div></main>;
}
...
function SettingsView({ profile, signOutPath, signedIn, onSaved }: { profile: Profile; signOutPath: string; signedIn: boolean; onSaved: () => void }) {
  ...
  return <section><p className="text-sm font-medium text-[#c8ff38]">Профіль</p><h1 className="mt-2 text-4xl font-semibold tracking-[-.045em] sm:text-5xl">Налаштування</h1><form className="mt-8 max-w-xl border border-white/9 bg-[#111412] p-6 sm:p-8" onSubmit={async (event) => { event.preventDefault(); setMessage("Зберігаю…"); const response = await fetch("/api/me", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ displayName: name, targetScore: target }) }); setMessage(response.ok ? "Збережено" : "Не вдалося зберегти"); if (response.ok) onSaved(); }}><label className="block"><span className="mb-2 block text-sm text-white/60">Ім’я</span><Input value={name} onChange={(event) => setName(event.target.value)} className="h-13 rounded-none border-white/10 bg-white/[.035] px-4 text-white" /></label><label className="mt-6 block"><span className="mb-3 block text-sm text-white/60">Бажаний бал: <strong className="text-[#c8ff38]">{target}</strong></span><input type="range" min="140" max="200" step="5" value={target} onChange={(event) => setTarget(Number(event.target.value))} className="w-full accent-[#c8ff38]" /></label><div className="mt-7 flex items-center gap-4"><Button className="rounded-none bg-white px-6 text-black hover:bg-[#c8ff38]">Зберегти</Button>{message && <span className="text-sm text-white/38">{message}</span>}</div></form>{signedIn ? <a href={signOutPath} target="_top" className="mt-5 inline-flex items-center gap-2 px-4 py-2 text-sm text-red-300/70 hover:bg-red-400/10 hover:text-red-300"><LogOut className="size-4" /> Вийти з акаунта</a> : <p className="mt-5 text-sm text-white/30">Зараз використовується тимчасовий гостьовий профіль на цьому пристрої.</p>}</section>;
}
