...
function Landing({ onStart }: { onStart: () => void }) {
  return <main className="relative min-h-screen overflow-hidden bg-[#080a09] text-white">
    ...
    <section className="relative z-10 mx-auto grid min-h-[calc(100vh-82px)] max-w-[1400px] items-center gap-12 px-5 pb-12 pt-8 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:px-12">
      <div className="max-w-[720px]"><div className="mb-7 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.035] px-3.5 py-2 text-sm text-white/55"><span className="size-1.5 rounded-full bg-[#c8ff38]" /> Формат НМТ-2026</div><h1 className="text-[clamp(3.4rem,7vw,7.3rem)] font-semibold leading-[0.88] tracking-[-0.075em]">Фокус на<br /><span className="text-[#c8ff38]">твої 200.</span></h1><p className="mt-8 max-w-xl text-lg leading-8 text-white/55 sm:text-xl">Тренуйся на офіційних завданнях і нових вправах за програмою НМТ, отримуй чесний результат і бач свій прогрес після кожного тесту.</p><Button size="lg" onClick={onStart} className="mt-10 h-14 rounded-full bg-[#c8ff38] px-7 text-base font-semibold text-[#10130b] hover:bg-[#d5ff65]">Почати підготовку <ArrowRight className="size-4" /></Button><div className="mt-14 grid max-w-xl grid-cols-3 border-t border-white/10 pt-7"><div><strong className="block text-2xl">7</strong><span className="text-sm text-white/36">предметів</span></div><div className="border-l border-white/10 pl-6"><strong className="block text-2xl">2 512</strong><span className="text-sm text-white/36">завдань</span></div><div className="border-l border-white/10 pl-6"><strong className="block text-2xl">60 хв</strong><span className="text-sm text-white/36">повна симуляція</span></div></div></div>
...
function TestsView({ stats, onStart }: { stats: SubjectStat[]; onStart: (slug: string, mode?: "quick" | "full" | "topic", topicId?: number) => void }) {
  ...
  const openSubject = (slug: string) => { setLoading(true); setTopics([]); setSelectedSlug(slug); };
  useEffect(() => {
    if (!selectedSlug) return;
    const controller = new AbortController();
    fetch(`/api/topics?subject=${selectedSlug}`, { cache: "no-store", signal: controller.signal }).then((response) => response.json()).then((payload) => setTopics(payload.topics ?? [])).finally(() => setLoading(false));
    return () => controller.abort();
  }, [selectedSlug]);
  if (selectedSubject) return <section><button onClick={() => { setSelectedSlug(null); setTopics([]); }} className="flex items-center gap-2 text-sm text-white/40 hover:text-white"><ArrowLeft className="size-4" /> Усі предмети</button><div className="mt-7 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-sm font-medium text-[#c8ff38]">Програма НМТ-2026</p><h1 className="mt-2 text-4xl font-semibold tracking-[-.045em] sm:text-5xl">{selectedSubject.name}</h1><p className="mt-3 max-w-2xl leading-7 text-white/40">Обери окрему тему для тренування або склади повний варіант у структурі НМТ.</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => onStart(selectedSubject.slug, "quick")} className="rounded-full border-white/10 bg-transparent text-white hover:bg-white/10 hover:text-white">Швидкий · 10</Button><Button onClick={() => onStart(selectedSubject.slug, "full")} className="rounded-full bg-[#c8ff38] text-black hover:bg-[#d5ff65]">Повний · {selectedSubject.count}</Button></div></div>{loading ? <div className="mt-8 rounded-[24px] border border-white/8 p-8 text-white/35">Завантажую теми…</div> : <div className="mt-8 grid gap-3 md:grid-cols-2">{topics.map((topic, index) => <article key={topic.id} className="group rounded-[22px] border border-white/8 bg-white/[.025] p-5 transition hover:border-[#c8ff38]/30"><div className="flex items-start gap-4"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-white/[.06] text-sm text-white/45">{String(index + 1).padStart(2, "0")}</span><div className="min-w-0 flex-1"><h2 className="font-medium leading-6">{topic.name}</h2><p className="mt-2 text-sm text-white/32">{topic.questionCount} завдань у банку</p></div></div><Button onClick={() => onStart(selectedSubject.slug, "topic", topic.id)} className="mt-5 w-full rounded-full bg-white text-black hover:bg-[#c8ff38]">Тренувати тему <ArrowRight className="size-4" /></Button></article>)}</div>}</section>;
  return <section><p className="text-sm font-medium text-[#c8ff38]">Банк · 2 512 завдань</p><h1 className="mt-2 text-4xl font-semibold tracking-[-.045em] sm:text-5xl">Обери предмет</h1><p className="mt-3 max-w-3xl leading-7 text-white/40">Теми впорядковано за офіційною програмою НМТ-2026. Повна симуляція відтворює кількість, типи завдань і шкалу 100–200.</p><div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{subjects.map(({ slug, name, icon: Icon, count, tone }) => { const stat = stats.find((item) => item.slug === slug); return <article key={slug} className="rounded-[24px] border border-white/8 bg-white/[.025] p-5 transition hover:-translate-y-1 hover:border-white/20"><div className="flex items-center justify-between"><span className={`grid size-11 place-items-center rounded-full subject-bg-${tone}`}><Icon className="size-5" /></span><span className="rounded-full border border-white/8 px-3 py-1 text-xs text-white/35">60 хв</span></div><h2 className="mt-7 text-xl font-medium">{name}</h2><p className="mt-1 text-sm text-white/32">{count} завдань у повному тесті</p><div className="mt-5 grid grid-cols-2 gap-2"><Button variant="outline" onClick={() => openSubject(slug)} className="rounded-full border-white/10 bg-transparent text-white hover:bg-white/10 hover:text-white">Теми</Button><Button onClick={() => onStart(slug, "full")} className="rounded-full bg-[#c8ff38] text-black hover:bg-[#d5ff65]">Повний · {count}</Button></div><div className="mt-5 flex items-center justify-between border-t border-white/8 pt-4 text-sm"><span className="text-white/35">Пройдено: {stat?.tests ?? 0}</span><span className="text-[#c8ff38]">{stat?.accuracy == null ? "Новий" : `${stat.accuracy}%`}</span></div></article>; })}</div></section>;
}
...
function TestPlayer({ test, onClose, onCompleted }: { test: ActiveTest; onClose: () => void; onCompleted: (result: TestResult) => void }) {
  ...
  const setGrouped = (groupIndex: number, marker: string) => {
    if (locked) return;
    const values = selected.split(";"); values[groupIndex] = markerKey(marker);
    setAnswers((current) => ({ ...current, [question.id]: values.join(";") }));
  };
  const toggleMultiple = (marker: string) => {
    if (locked) return;
    const key = markerKey(marker); const values = selected.split(";").filter(Boolean);
  ...
  const revealAnswer = async () => {
    ...
    setRevealing(false);
    if (response.ok) setReveals((current) => ({ ...current, [question.id]: payload }));
  };
  ...
  const answerControl = question.question_type === "numeric" ? <Input disabled={locked} inputMode="decimal" value={selected} onChange={(event) => setAnswers((current) => ({ ...current, [question.id]: event.target.value }))} placeholder="Уведи числову відповідь" className="mt-8 h-14 rounded-2xl border-white/10 bg-white/[.035] px-4 text-lg text-white" /> : question.question_type === "matching" ? <div className="mt-8 space-y-3">{groups[0]?.options.map((left) => <div key={left.marker} className="grid gap-3 rounded-2xl border border-white/9 bg-white/[.025] p-4 sm:grid-cols-[1fr_240px] sm:items-center"><p className="text-white/72"><strong className="mr-2 text-[#c8ff38]">{left.marker}</strong>{left.text}</p><select disabled={locked} aria-label={`Відповідь для ${left.marker}`} value={pairs[left.marker] ?? ""} onChange={(event) => setPair(left.marker, event.target.value)} className="h-11 rounded-xl border border-white/10 bg-[#171a18] px-3 text-white disabled:opacity-60"><option value="">Обрати пару</option>{groups[1]?.options.map((right) => <option key={right.marker} value={markerKey(right.marker)}>{right.marker} — {right.text}</option>)}</select></div>)}</div> : question.question_type === "ordering" ? <div className="mt-8 space-y-3">{options.map((_, order) => <div key={order} className="grid grid-cols-[88px_1fr] items-center gap-3"><span className="text-sm text-white/38">{order + 1} місце</span><select disabled={locked} value={pairs[String(order + 1)] ?? ""} onChange={(event) => setPair(String(order + 1), event.target.value)} className="h-12 rounded-xl border border-white/10 bg-[#171a18] px-3 text-white disabled:opacity-60"><option value="">Обрати подію</option>{options.map((option) => <option key={option.marker} value={markerKey(option.marker)}>{option.marker} — {option.text}</option>)}</select></div>)}</div> : question.question_type === "type_6" ? <div className="mt-8 space-y-5">{groups.map((group, groupIndex) => <div key={group.title} className="rounded-2xl border border-white/9 bg-white/[.025] p-4"><p className="mb-3 font-medium text-white/70">{group.title}</p><div className="grid gap-2">{group.options.map((option) => { const chosen = selected.split(";")[groupIndex] === markerKey(option.marker); const correct = reveal?.correctAnswer.split(";")[groupIndex] === markerKey(option.marker); return <button disabled={locked} key={option.marker} onClick={() => setGrouped(groupIndex, option.marker)} className={`rounded-xl border px-4 py-3 text-left disabled:opacity-100 ${reveal && correct ? "border-emerald-400/60 bg-emerald-400/10" : reveal && chosen ? "border-red-400/55 bg-red-400/10" : chosen ? "border-[#c8ff38]/60 bg-[#c8ff38]/10" : "border-white/8"}`}>{option.marker} — {option.text}</button>; })}</div></div>)}</div> : <div className="mt-8 grid gap-3">{options.map((option) => { const optionKey = markerKey(option.marker); const chosen = question.question_type === "multiple_choice" ? selected.split(";").includes(optionKey) : selected === optionKey; const correct = reveal?.correctAnswer.split(";").includes(optionKey); return <button disabled={locked} key={option.marker} onClick={() => question.question_type === "multiple_choice" ? toggleMultiple(option.marker) : setAnswers((current) => ({ ...current, [question.id]: optionKey }))} className={`flex min-h-14 items-center gap-4 rounded-2xl border px-4 py-3 text-left transition disabled:opacity-100 ${reveal && correct ? "border-emerald-400/60 bg-emerald-400/10" : reveal && chosen ? "border-red-400/55 bg-red-400/10" : chosen ? "border-[#c8ff38]/60 bg-[#c8ff38]/10" : "border-white/9 bg-white/[.025] hover:border-white/20"}`}><span className={`grid size-8 shrink-0 place-items-center rounded-full text-sm font-semibold ${reveal && correct ? "bg-emerald-400 text-black" : reveal && chosen ? "bg-red-400 text-black" : chosen ? "bg-[#c8ff38] text-black" : "bg-white/8 text-white/55"}`}>{chosen && question.question_type === "multiple_choice" ? <Check className="size-4" /> : option.marker}</span><span className="leading-6 text-white/75">{option.text || "Варіант із зображенням"}</span>{option.images?.map((src) => <img key={src} src={src} alt="Варіант відповіді" className="ml-auto max-h-20 max-w-32 rounded bg-white object-contain p-1" />)}</button>; })}</div>;
  ...
  return <main className="min-h-screen bg-[#080a09] px-4 py-5 text-white sm:px-8"><div className="mx-auto max-w-4xl"><header className="flex items-center justify-between"><button onClick={onClose} className="flex items-center gap-2 text-sm text-white/45 hover:text-white"><ArrowLeft className="size-4" /> Вийти</button><div className="flex items-center gap-3 text-sm"><span className="text-white/38">{index + 1} / {test.questions.length}</span>{test.mode === "full" && <span className="rounded-full border border-[#c8ff38]/20 bg-[#c8ff38]/8 px-3 py-1 font-mono text-[#c8ff38]">{String(Math.floor(secondsLeft / 60)).padStart(2, "0")}:{String(secondsLeft % 60).padStart(2, "0")}</span>}</div></header><Progress value={((index + 1) / test.questions.length) * 100} className="mt-5 h-1.5 bg-white/8 [&_[data-slot=progress-indicator]]:bg-[#c8ff38]" /><article className="mt-8 rounded-[30px] border border-white/10 bg-[#111412] p-5 sm:p-9"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-sm text-[#c8ff38]">{test.subjectName} · {question.topic}</p><span className="text-xs uppercase tracking-[.12em] text-white/25">{question.question_type.replaceAll("_", " ")}</span></div><h1 className="mt-5 whitespace-pre-line text-xl font-medium leading-8 sm:text-2xl">{question.prompt.replaceAll("\\\\(", "").replaceAll("\\\\)", "")}</h1>{images.length > 0 && <div className="mt-6 grid gap-3 sm:grid-cols-2">{images.map((src) => <img key={src} src={src} alt="Ілюстрація до завдання" className="max-h-72 rounded-2xl bg-white object-contain p-2" />)}</div>}{answerControl}{reveal && <div className={`mt-6 rounded-2xl border p-5 ${reveal.isCorrect ? "border-emerald-400/25 bg-emerald-400/[.07]" : "border-amber-300/25 bg-amber-300/[.06]"}`}><div className="flex items-center justify-between gap-4"><p className={`font-medium ${reveal.isCorrect ? "text-emerald-300" : "text-amber-200"}`}>{reveal.isCorrect ? "Правильно" : "Перевір відповідь"}</p><span className="text-sm text-white/42">{reveal.points}/{reveal.maxPoints} тестових балів</span></div><p className="mt-3 text-sm text-white/65">Правильна відповідь: <strong className="text-white">{prettyAnswer}</strong></p>{reveal.explanation && <p className="mt-3 whitespace-pre-line text-sm leading-6 text-white/42">{reveal.explanation}</p>}</div>}<div className="mt-8 flex items-center justify-between"><Button variant="ghost" disabled={index === 0} onClick={() => setIndex((value) => value - 1)} className="rounded-full text-white/50 hover:bg-white/10 hover:text-white">Назад</Button>{!reveal ? <Button disabled={!ready || revealing} onClick={revealAnswer} className="h-12 rounded-full bg-white px-6 text-black hover:bg-white/85">{revealing ? "Перевіряю…" : "Показати правильну відповідь"}</Button> : index < test.questions.length - 1 ? <Button onClick={() => setIndex((value) => value + 1)} className="h-12 rounded-full bg-white px-6 text-black hover:bg-white/85">Далі <ArrowRight className="size-4" /></Button> : <Button disabled={sending} onClick={finish} className="h-12 rounded-full bg-[#c8ff38] px-6 text-black hover:bg-[#d5ff65]">{sending ? "Перевіряю…" : "Завершити тест"}</Button>}</div></article><p className="mt-5 text-center text-xs text-white/22">{question.attribution}</p></div></main>;
}

function ResultScreen({ result, onContinue }: { result: TestResult; onContinue: () => void }) {
  const official = result.scaleKind === "official-2026";
  return <main className="grid min-h-screen place-items-center bg-[#080a09] p-5 text-white"><div className="w-full max-w-lg rounded-[32px] border border-white/10 bg-[#111412] p-8 text-center"><span className="mx-auto grid size-16 place-items-center rounded-full bg-[#c8ff38]/10 text-[#c8ff38]"><Trophy className="size-7" /></span><p className="mt-7 text-sm text-white/38">{official ? "Результат за таблицею НМТ-2026" : "Тренувальний індекс"}</p><h1 className="mt-2 text-7xl font-semibold tracking-[-.07em] text-[#c8ff38]">{result.score ?? "—"}</h1>{official && !result.passed && <p className="mt-3 font-medium text-amber-200">Пороговий бал не подолано</p>}<p className="mt-4 text-lg text-white/55">{result.rawScore} тестових балів із {result.rawMax}</p><p className="mt-1 text-sm text-white/35">повністю виконано {result.correct} із {result.total} завдань</p><p className="mt-6 text-sm leading-6 text-white/32">{official ? "Переведення 100–200 виконано за чинною таблицею МОН для НМТ-2026. Часткові бали нараховано за офіційними схемами УЦОЯО." : "Для тренування окремої теми показано індекс 100–200; офіційна таблиця застосовується лише до повного предметного тесту."}</p><Button onClick={onContinue} className="mt-8 h-13 w-full rounded-full bg-white text-black hover:bg-white/85">До результатів <ArrowRight className="size-4" /></Button></div></main>;
}
...
export default function VektoApp({ initialIdentity, signOutPath }: { initialIdentity: { userId: string; email: string } | null; signInPath: string; signOutPath: string }) {
  ...
  if (screen === "app" && ready) return <><Dashboard data={dashboard} view={view} setView={setView} onStart={startTest} onRefresh={refresh} signOutPath={signOutPath} signedIn={Boolean(initialIdentity)} />{loadingTest && <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 text-white backdrop-blur-sm"><div className="rounded-full border border-white/10 bg-[#151815] px-5 py-3 text-sm">Формую варіант за структурою НМТ…</div></div>}</>;
