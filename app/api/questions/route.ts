...
async function seedSubject(db: D1Database, subject: string, records: QuestionRecord[]) {
  ...
  if (!catalog) return;
  const current = await db.prepare("SELECT COUNT(*) AS count FROM questions WHERE subject_slug = ?1").bind(subject).first<{ count: number }>();
  ...
  const questionStatements = records.map((record) => db.prepare(
    `INSERT OR IGNORE INTO questions (id, external_id, subject_slug, topic_id, year, session, position, question_type, prompt, options_json, correct_answer, explanation, images_json, source_url, official_pdf_url, attribution)
