...
export async function GET(request: Request) {
  ...
  try {
    ...
    const rows = await db.prepare(`SELECT t.id, t.name, COUNT(q.id) AS question_count
      FROM topics t LEFT JOIN questions q ON q.topic_id=t.id
      WHERE t.subject_slug=?1 GROUP BY t.id, t.name`).bind(subject).all<{ id: number; name: string; question_count: number }>();
    const counts = new Map(rows.results.map((row) => [row.name, row]));
    const topics = (officialTopicSections[subject] ?? []).map((name) => ({ id: counts.get(name)?.id, name, questionCount: counts.get(name)?.question_count ?? 0 })).filter((topic) => topic.id && topic.questionCount > 0);
    return Response.json({ subject, subjectName: catalog.name, topics }, noStore());
  } catch (error) {
