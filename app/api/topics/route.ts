...
import { ensureSubjectSeeded, QUESTION_BANK_SIZE, subjectCatalog, topicTaxonomy } from "@/lib/question-bank";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const sections = (topicTaxonomy[subject] ?? []).map((section) => ({
      ...
      topics: section.topics.map((name) => ({ id: counts.get(name)?.id, name, questionCount: counts.get(name)?.question_count ?? 0 })).filter((topic) => topic.id && topic.questionCount > 0),
    })).filter((section) => section.questionCount > 0);
    return Response.json({ subject, subjectName: catalog.name, bankSize: QUESTION_BANK_SIZE, blueprint: catalog.blueprint, sections }, noStore());
  } catch (error) {
